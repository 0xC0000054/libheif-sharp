...
namespace LibHeifSharp
{
    ...
    public sealed class TransformedRegionPolygon : TransformedRegionGeometry
    {
        private readonly ReadOnlyCollection<TransformedPolygonPoint> points;

        /// <summary>
        /// Initializes a new instance of the <see cref="TransformedRegionPolygon"/> class.
        /// </summary>
        ...
        /// </exception>
        internal TransformedRegionPolygon(heif_region region, HeifItemId imageId) : base(RegionGeometryType.Polygon)
        {
            var points = Array.Empty<TransformedPolygonPoint>();

            int count = LibHeifNative.heif_region_get_polygon_num_points(region);
            ...
            if (count > 0)
            {
                ...
                try
                {
                    ...
                    unsafe
                    {
                        ...
                        {
                            var error = LibHeifNative.heif_region_get_polygon_points_transformed(region,
                                                                                                 ptr,
                                                                                                 imageId);
                            error.ThrowIfError();
                        }
        ...
        {
            var builder = new StringBuilder("transformed_polygon [");
